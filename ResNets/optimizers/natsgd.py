...
import numpy as np
from utils import nat_compression


class NatSGD(Optimizer):
    r"""Implements stochastic gradient descent with natural compression
    """
    ...
    def __init__(self, params, lr=required, momentum=0, dampening=0,
        ...
        super(NatSGD, self).__init__(params, defaults)
    ...
    def __setstate__(self, state):
        super(NatSGD, self).__setstate__(state)
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group['params']):
                ...
                if 'parallel_buffer' not in param_state:
                    ...
                    nat_update = nat_compression(torch.clone(d_p).detach())
                    self.n_bit += nat_update.numel() * 9
                    par_buf.add_(nat_update, alpha=1 / self.n_fake_workers)
                ...
                if self.i_worker % self.n_fake_workers == self.n_fake_workers - 1:
                    ...
                    p.add_(nat_compression(par_buf), alpha=-group['lr'])
                    self.n_bit += par_buf.numel() * 9 * self.n_fake_workers
                    par_buf.zero_()
            if self.i_worker % self.n_fake_workers == self.n_fake_workers - 1:
