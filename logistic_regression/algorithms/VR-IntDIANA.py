...
from utils import set_seed, int_quant, rotate_func
import numpy as onp
import argparse
...
if problem_type == 'cvx':
    lr0 = 1 / (4 * (L + 3 * max_Li_batch / (2 * n_workers) + 3 * max_Li_batch / (16 * n_workers)))
else:
    lr0 = 1 / (2 * (L + 3 * max_Li_batch / (2 * n_workers) + 3 * max_Li_batch / (16 * n_workers)))
...
for seed in seeds_i:
    ...
    if rank == 0:
        lr_handler.set_lr()
        alpha_handler.reset_alpha()

    n_bits = 0
    n_grad = 0
    for k in range(it_max):
        if rank == 0:
            alpha = alpha_handler.get_alpha()
        ...
        if k == 0:
            Qi_k = alpha * (local_sgrad - hi_k)
            n_bit = 0
        else:
            Qi_k = int_quant(alpha * (local_sgrad - hi_k))
            max_abs_i = onp.max(onp.abs(Qi_k))
            if max_abs_i < 1:
                n_bit = 1
            else:
                n_bit = 1 + onp.ceil(onp.log2(max_abs_i))

        n_bit_all = comm.allreduce(n_bit, op=MPI.SUM)
        n_bits += n_bit_all

        hi_k_next = hi_k + Qi_k / alpha
        hi_k = onp.copy(hi_k_next)

        Q_k = comm.allreduce(Qi_k)

        max_abs_sum = onp.max(onp.abs(Q_k))
        if max_abs_sum < 1:
            sum_n_bit = 1
        else:
            sum_n_bit = 1 + onp.ceil(onp.log2(max_abs_sum))
        ...
        if k % trace_period == 0:
            ...
            if rank == 0:
                ...
                trace_handler.update_max_value(seed=seed, max_val=max_val, max_val_sum=max_val_sum, alpha=alpha)
        ...
        g_k = h_k + Q_k / (alpha * n_workers)
        x_prev = onp.copy(x)
        x = x - lr * g_k

        if rank == 0:
            if k == 0:
                alpha_handler.set_init_r_k(x, x_prev)

            alpha_handler.set_alpha(x, x_prev)

        h_k_next = h_k + Q_k / (alpha * n_workers)
